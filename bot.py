...
ZainOnBot = Client("AntiChannelBot",
...
@ZainOnBot.on_message(custom_message_filter & filters.group & custom_chat_filter)
async def main_handler(bot, message):
    ...
    if res:
        ...
        await message.reply_text(text=f"{mention} has been banned😈.\n\n馃挕 He can write only with his profile but not through other channels.",
                                 reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Unban", callback_data=f"unban_{chat_id}_{a_id}")]]),
                              )
    await message.delete()


@ZainOnBot.on_message(filters.command(["start"]) & filters.private)
async def start_handler(bot, message):
    await message.reply_text(text="""😍Hey! Just add me to the chat, and I will block the channels that write to the chat,

check /help for more👈🏻.""",
                             reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Love Channel", url=f"https://t.me/About_Zain"),
                                                                 InlineKeyboardButton("Support Group", url=f"https://t.me/ChatOnMasti")]]),
                             disable_web_page_preview=True)

@ZainOnBot.on_message(filters.command(["help"]) & filters.private)
...
for more help ask at @The_Eternity_Soul""",
                             reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Love Channel", url=f"https://t.me/About_Zain"),
                                                                 InlineKeyboardButton("Support Group", url=f"https://t.me/ChatOnMasti")]]),
                             disable_web_page_preview=True)
...
@ZainOnBot.on_message(filters.command(["ban"]) & filters.group)
async def cban_handler(bot, message):
    ...
    try:
        ...
        if res:
            ...
            await message.reply_text("Invalid Channel id, 馃挕check channel id")
    except Exception as e:
        print(e)

@ZainOnBot.on_message(filters.command(["unban"]) & filters.group)
...
@ZainOnBot.on_message(filters.command(["add_whitelist"]) & filters.group)
...
@ZainOnBot.on_message(filters.command(["del_whitelist"]) & filters.group)
...
@ZainOnBot.on_message(filters.command(["show_whitelist"]) & filters.group)
async def del_whitelist_handler(bot, message):
    chat_id = message.chat.id
    user = await bot.get_chat_member(chat_id, message.from_user.id)
    if user.status == "creator" or user.status == "administrator":
